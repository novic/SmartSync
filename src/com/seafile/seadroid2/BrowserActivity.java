...
public class BrowserActivity extends SherlockFragmentActivity implements
	...
	private class PendingUploadInfo {
		...
		public PendingUploadInfo(String repoID, String repoName,
				String targetDir, String localFilePath, boolean isUpdate) {
			this.repoID = repoID;
	...
	public void addUpdateTask(String repoID, String repoName, String targetDir,
			String localFilePath) {
		if (txService != null) {
			txService.addUploadTask(account, repoID, repoName, targetDir,
					localFilePath, true);
		} else {
			PendingUploadInfo info = new PendingUploadInfo(repoID, repoName,
					targetDir, localFilePath, true);
			pendingUploads.add(info);
		}
	}

	private void addUploadTask(String repoID, String repoName,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (server == null) {
			...
			SharedPreferences sharedPref = getSharedPreferences(
					AccountsActivity.SHARED_PREF_NAME, Context.MODE_PRIVATE);
			String latest_server = sharedPref.getString(
					AccountsActivity.SHARED_PREF_SERVER_KEY, null);
			String latest_email = sharedPref.getString(
					AccountsActivity.SHARED_PREF_EMAIL_KEY, null);
			String latest_token = sharedPref.getString(
					AccountsActivity.SHARED_PREF_TOKEN_KEY, null);

			if (latest_server != null) {
				account = new Account(latest_server, latest_email, null,
						latest_token);
			} else {
		...
		if (savedInstanceState != null) {
			...
			tabsFragment = (TabsFragment) getSupportFragmentManager()
					.findFragmentByTag(TABS_FRAGMENT_TAG);
			uploadTasksFragment = (UploadTasksFragment) getSupportFragmentManager()
					.findFragmentByTag(UPLOAD_TASKS_FRAGMENT_TAG);

			fetchFileDialog = (FetchFileDialog) getSupportFragmentManager()
					.findFragmentByTag(OPEN_FILE_DIALOG_FRAGMENT_TAG);

			appChoiceDialog = (AppChoiceDialog) getSupportFragmentManager()
					.findFragmentByTag(CHOOSE_APP_DIALOG_FRAGMENT_TAG);
			...
			SslConfirmDialog sslConfirmDlg = (SslConfirmDialog) getSupportFragmentManager()
					.findFragmentByTag(SslConfirmDialog.FRAGMENT_TAG);
			...
			getSupportFragmentManager().beginTransaction()
					.add(R.id.content_frame, tabsFragment, TABS_FRAGMENT_TAG)
					.commit();
			getSupportFragmentManager()
					.beginTransaction()
					.add(R.id.content_frame, uploadTasksFragment,
							UPLOAD_TASKS_FRAGMENT_TAG).commit();
			getSupportFragmentManager().beginTransaction()
					.detach(uploadTasksFragment).commit();
	...
	ServiceConnection mConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			for (PendingUploadInfo info : pendingUploads) {
				txService.addUploadTask(account, info.repoID, info.repoName,
						info.targetDir, info.localFilePath, info.isUpdate);
			}
			pendingUploads.clear();

			if (currentSelectedItem.equals(UPLOAD_TASKS_VIEW)
					&& uploadTasksFragment != null
					&& uploadTasksFragment.isReady()) {
		...
		case R.id.refresh:
			...
			if (getCurrentTabName().equals(LIBRARY_TAB)) {
				if (navContext.inRepo()) {
					SeafRepo repo = dataManager.getCachedRepoByID(navContext
							.getRepoID());
					if (repo.encrypted
							&& !DataManager.getRepoPasswordSet(repo.id)) {
						String password = DataManager.getRepoPassword(repo.id);
						showPasswordDialog(repo.name, repo.id,
								new TaskDialog.TaskDialogListener() {
									@Override
									public void onTaskSuccess() {
										tabsFragment.getReposFragment()
												.refreshView(true);
									}
								}, password);
	...
	private void showNewDirDialog() {
		...
		dialog.setTaskDialogLisenter(new TaskDialog.TaskDialogListener() {
			@Override
			public void onTaskSuccess() {
				showToast("Sucessfully created folder "
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							case 0:
								Intent intent = new Intent(
										...
										MultiFileChooserActivity.class);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case PICK_FILES_REQUEST:
			if (resultCode == RESULT_OK) {
				String[] paths = data
						.getStringArrayExtra(MultiFileChooserActivity.MULTI_FILES_PATHS);
				if (paths == null)
					return;
				showToast(getString(R.string.added_to_upload_tasks));
				for (String path : paths) {
					addUploadTask(navContext.getRepoID(),
							navContext.getRepoName(), navContext.getDirPath(),
							path);
				}
			}
			break;
		...
		case PICK_FILE_REQUEST:
			if (resultCode == RESULT_OK) {
				...
				addUploadTask(navContext.getRepoID(), navContext.getRepoName(),
	...
	public void onFileSelected(SeafDirent dirent) {
		...
		final String filePath = Utils.pathJoin(navContext.getDirPath(),
				fileName);
	...
	public void onBackPressed() {
		...
		if (currentSelectedItem == FILES_VIEW
				&& getCurrentTabName().equals(LIBRARY_TAB)) {
			if (navContext.inRepo()) {
	...
	public void showFile(File file) {
		...
		try {
			...
			new OpenAsDialog(file).show(getSupportFragmentManager(),
					"OpenAsDialog");
		...
		/*
		 * String chooser_title = getString(R.string.open_with); Intent chooser
		 * = Intent.createChooser(open, chooser_title);
		 * 
		 * if (open.resolveActivity(getPackageManager()) != null) {
		 * startActivity(chooser); return; } else {
		 * showToast(R.string.activity_not_found); return; }
		 */

	}

	/**
	 * Export a file. 1. first ask the user to choose an app 2. then download
	 * the latest version of the file 3. start the choosen app
	 * 
	 * @param fileName
	 *            The name of the file to share in the current navcontext
	 */
	...
	private void chooseExportApp(final String repoName, final String repoID,
			final String path) {
		...
		dialog.init(getString(R.string.export_file), infos,
				new AppChoiceDialog.OnItemSelectedListener() {
					@Override
					...
					public void onAppSelected(ResolveInfo appInfo) {
						...
						fetchFileAndExport(appInfo, sendIntent, repoName,
								repoID, path);
					}
	...
	private void fetchFileAndExport(final ResolveInfo appInfo,
			final Intent intent, final String repoName, final String repoID,
			final String path) {

		fetchFileDialog = new FetchFileDialog();
		fetchFileDialog.init(repoName, repoID, path,
		...
		fetchFileDialog.show(getSupportFragmentManager(),
	...
	private void chooseShareApp(final String repoID, final String path,
			final boolean isdir) {
		...
		String title = getString(isdir ? R.string.share_dir_link
				: R.string.share_file_link);

		AppChoiceDialog dialog = new AppChoiceDialog();
		dialog.addCustomAction(0,
				getResources().getDrawable(R.drawable.copy_link),
		...
		dialog.init(title, infos, new AppChoiceDialog.OnItemSelectedListener() {
			...
			public void onCustomActionSelected(CustomAction action) {
				...
				gdialog.setTaskDialogLisenter(new TaskDialogListener() {
					@Override
					public void onTaskSuccess() {
						// TODO: generate a share link through SeafConnection
						// and copy
						// it to clipboard
						ClipboardManager clipboard = (ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
						clipboard.setText(gdialog.getLink());
						// ClipData clip =
						// ClipData.newPlainText("seafile shared link",
						// gdialog.getLink());
	...
	private void doRename(String repoID, String repoName, String path,
			boolean isdir) {
	...
	private void onFileUploaded(int taskID) {
		...
		if (getCurrentTabName().equals(LIBRARY_TAB)
			...
			tabsFragment.getReposFragment().refreshView(true);
			String verb = getString(info.isUpdate ? R.string.updated
					: R.string.uploaded);
	...
	private void onFileDownloaded(int taskID) {
		...
		if (fetchFileDialog != null && fetchFileDialog.getTaskID() == taskID) {
			...
			if (getCurrentTabName().equals(LIBRARY_TAB)
					...
					&& Utils.getParentPath(info.pathInRepo).equals(
			...
			if (getCurrentTabName().equals(LIBRARY_TAB)
				...
				showPasswordDialog(repoName, repoID,
						new TaskDialog.TaskDialogListener() {
							@Override
							public void onTaskSuccess() {
								txService.addDownloadTask(account, repoName,
										repoID, path);
							}
						});
				return;
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (type.equals(TransferService.BROADCAST_FILE_DOWNLOAD_PROGRESS)) {
				...
				onFileDownloadProgress(taskID);
					...
					.equals(TransferService.BROADCAST_FILE_DOWNLOAD_SUCCESS)) {
				int taskID = intent.getIntExtra("taskID", 0);
				onFileDownloaded(taskID);

			} else if (type
					.equals(TransferService.BROADCAST_FILE_DOWNLOAD_FAILED)) {
				int taskID = intent.getIntExtra("taskID", 0);
				...
				onFileUploaded(taskID);
					...
					.equals(TransferService.BROADCAST_FILE_UPLOAD_FAILED)) {
				int taskID = intent.getIntExtra("taskID", 0);
				onFileUploadFailed(taskID);

			} else if (type
					.equals(TransferService.BROADCAST_FILE_UPLOAD_PROGRESS)) {
				int taskID = intent.getIntExtra("taskID", 0);
				onFileUploadProgress(taskID);
			} else if (type
					.equals(TransferService.BROADCAST_FILE_UPLOAD_CANCELLED)) {
				int taskID = intent.getIntExtra("taskID", 0);
